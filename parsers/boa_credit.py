...
Credit-card statements often use a dual-date format: ``MM/DD MM/DD``
(transaction date + posting date). We accept either ``MM/DD/YY`` or
``MM/DD`` for the first token, and skip past a second date token if
present.
"""
...
_NOISE = ("TOTAL", "ACCOUNT SUMMARY", "PAGE", "BALANCE", "STATEMENT", "CONTINUED", "BEGINNING", "ENDING", "MINIMUM")

_PAYMENT_KEYWORDS = ("PAYMENT", "THANK YOU", "AUTOPAY", "MOBILE PAYMENT", "ONLINE PAYMENT")
...
def parse(path: Path) -> pd.DataFrame:
    ...
    with pdfplumber.open(str(path)) as pdf:
        ...
        for page in pdf.pages:
            ...
            for line in text.split("\n"):
                ...
                date_raw = parts[0]
                # If there's a second date token (posting date), skip it.
                ...
                # Payments (e.g. "PAYMENT - THANK YOU") and explicit negatives
                # are inflows on a credit card statement.
    ...
    return to_canonical(df, "boa_credit", "BoA Credit Card")
